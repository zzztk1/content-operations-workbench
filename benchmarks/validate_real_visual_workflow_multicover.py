"""Real LLM + real image API: full workflow with multiple cover candidates."""
...
def main() -> None:
    ...
    with LangGraphMediaAgentEngine(settings=settings) as engine:
        result = engine.run_workflow(
            ...
            approval_note="real-visual-multicover-check",
            reviewer_threshold=21,
            max_revisions=1,
            run_id="realvisualmulti001",
            thread_id="realvisualmulti001",
            request_id="real-visual-multi-001",
            runtime_overrides={"image_mock": False, "cover_candidate_count": cover_n},
        )
    ...
    primary_path = Path(image_asset.get("local_path", "")) if image_asset.get("local_path") else None
    ...
    checks.extend(
        [
            ...
            {
                "name": "primary_image_file_exists",
                "actual": bool(primary_path and primary_path.exists()),
                "expected": True,
                "pass": bool(primary_path and primary_path.exists()),
            },
            {
                "name": "all_candidate_files_exist",
                "actual": all(
                    Path(candidate.get("local_path", "")).exists()
    ...
    out_path = PROJECT_ROOT / "benchmarks" / "latest_real_visual_workflow_multicover_report.json"
